         ...
         return ele.contents.map((el, index) => {
            return (
              <div key={index} className="remark_vipDealer">
                <p>
                  <span style={{color:theme === "light" ? "#858585" : "#B5B5B6",fontWeight:400}}>{el.title}</span>
                  <span style={{color:theme === "light" ? "#0B0B0C" : "#ffffff",fontWeight:500}}>{el.text}</span>
                </p>
                {/* VIP dealer check */}
                <Checkbox label="VIP Dealer" drawerType={drawerType} handleChecked={handleChecked} checked={checked} />
              </div>
            );
          });
        }
